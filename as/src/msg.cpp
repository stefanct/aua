...
void Msg::print_msg(MSG_LEVEL lvl, const std::string& file, int line, const string& msg) {
	if (lvl <= this->lvl) {
...
void Msg::flush() {
	...
	for (vector<_msg>::iterator iter = msgs.begin(); iter != msgs.end(); iter++) {
		stringstream ss;
		ss << iter->get_file() << ":" << iter->get_line() << ": "
...
void Msg::err_number_args(const string& file, int line, const string& instruction, int exp,
		int found) {
	stringstream ss;
...
void Msg::err_out_of_range(const string& file, int line, int value, int min, int max) {
	stringstream ss;
...
void Msg::warn_const_as_offset(const string& file, int line){
	print_msg(WARN, file, line, "Using reference to constant value as relative address");
}

void Msg::warn_illegal_const(const string& file, int line, const string& str_constant){
	stringstream ss;
...
void Msg::wall_signed_overflow(const string& file, int line, int value){
	stringstream ss;
	ss << "Signed value expected. The unsigned value " << value << " will overflow and be interpreted as negative integer";
